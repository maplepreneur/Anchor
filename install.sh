...
REPO_SLUG="maplepreneur/Mountie"
...
IN_SOURCE_TREE=0
if [[ -n "$SCRIPT_DIR" && -f "$SCRIPT_DIR/Cargo.toml" ]] && grep -q 'name = "mountie"' "$SCRIPT_DIR/Cargo.toml" 2>/dev/null; then
  IN_SOURCE_TREE=1
  ROOT="$SCRIPT_DIR"
else
  ROOT=""
...
install_user_from_binary() {
  ...
  install -m 755 "$binary" "${bin_dir}/mountie"

  local desktop_src icon_src
  if [[ "$IN_SOURCE_TREE" -eq 1 ]]; then
    desktop_src="${ROOT}/resources/com.voxelnorth.Mountie.desktop"
    icon_src="${ROOT}/resources/icons/com.voxelnorth.Mountie.png"
  else
    desktop_src="$(mktemp)"
    icon_src="$(mktemp --suffix=.png)"
    curl -fsSL "${RAW_BASE}/resources/com.voxelnorth.Mountie.desktop" -o "$desktop_src"
    curl -fsSL "${RAW_BASE}/resources/icons/com.voxelnorth.Mountie.png" -o "$icon_src"
  fi

  install -m 644 "$desktop_src" "${apps_dir}/com.voxelnorth.Mountie.desktop"
  install -m 644 "$icon_src" "${icon_dest}/com.voxelnorth.Mountie.png"
  ...
  ok "Mountie installed. Run: mountie"
  printf '    Or open “Mountie” from the application menu.\n'
}
...
install_deb_file() {
  ...
  ok "Mountie installed system-wide. Run: mountie"
}
...
download_latest_deb() {
  ...
  # Prefer: mountie_*_${arch}.deb
  name="$(printf '%s' "$json" | grep -oE "\"name\":\\s*\"mountie_[^\"]+_${arch}\\.deb\"" | head -1 | sed -E 's/.*"([^"]+)".*/\1/' || true)"
  asset_url="$(printf '%s' "$json" | grep -oE "\"browser_download_url\":\\s*\"[^\"]*mountie_[^\"]+_${arch}\\.deb\"" | head -1 | sed -E 's/.*"([^"]+)".*/\1/' || true)"
  ...
  info "Downloading ${name:-mountie.deb}…"
...
build_from_source() {
  ...
  binary="${workdir}/target/release/mountie"
  ...
  if [[ "$MODE" == "system" ]] && ! is_debian_like; then
    ...
    sudo_run install -m 755 "$binary" /usr/local/bin/mountie
    sudo_run mkdir -p \
      /usr/local/share/applications \
      /usr/local/share/icons/hicolor/256x256/apps
    if [[ -f "${workdir}/resources/com.voxelnorth.Mountie.desktop" ]]; then
      sudo_run install -m 644 \
        "${workdir}/resources/com.voxelnorth.Mountie.desktop" \
        /usr/local/share/applications/
      sudo_run install -m 644 \
        "${workdir}/resources/icons/com.voxelnorth.Mountie.png" \
        /usr/local/share/icons/hicolor/256x256/apps/
    fi
...
build_and_install_deb_from_source() {
  ...
  local deb
  deb="$(ls -1t "${workdir}"/dist/mountie_*.deb 2>/dev/null | head -1 || true)"
...
uninstall_user() {
  ...
  rm -f "${PREFIX_USER}/bin/mountie" "${PREFIX_USER}/bin/anchor"
  rm -f "${PREFIX_USER}/share/applications/com.voxelnorth.Mountie.desktop"
  rm -f "${PREFIX_USER}/share/applications/com.voxelnorth.Anchor.desktop"
  rm -f "${PREFIX_USER}/share/icons/hicolor/256x256/apps/com.voxelnorth.Mountie.png"
  ...
  warn "Web apps and data under ~/.local/share/mountie (and ~/.local/share/anchor) were left in place"
}

uninstall_system() {
  if is_debian_like && dpkg -l mountie 2>/dev/null | grep -q '^ii'; then
    info "Removing Debian package…"
    sudo_run apt-get remove -y mountie || sudo_run dpkg -r mountie
    ok "Package removed"
    return
  fi
  if [[ -x /usr/local/bin/mountie ]]; then
    info "Removing /usr/local install…"
    sudo_run rm -f /usr/local/bin/mountie
    sudo_run rm -f /usr/local/share/applications/com.voxelnorth.Mountie.desktop
    sudo_run rm -f /usr/local/share/icons/hicolor/256x256/apps/com.voxelnorth.Mountie.png
    ok "System install removed"
    return
  fi
  warn "No system install of Mountie found"
}
...
case "$MODE" in
  ...
  user)
    if [[ "$IN_SOURCE_TREE" -eq 1 ]]; then
      ...
      install_user_from_binary "${ROOT}/target/release/mountie"
    else
      build_from_source
    fi
    exit 0
  ...
  auto)
    ...
    if [[ "$IN_SOURCE_TREE" -eq 1 ]] && is_debian_like; then
      local_deb="$(ls -1t "${ROOT}"/dist/mountie_*.deb 2>/dev/null | head -1 || true)"
      if [[ -n "${local_deb:-}" ]]; then
